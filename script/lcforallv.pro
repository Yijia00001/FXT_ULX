...
if keyword_set(saveps) then begin
	...
	set_plot, 'ps'
	epsname = 'ulx_lcallv.eps'
	device, filename=epsname, /encapsulated, xsize=15, ysize=20
endif
...
pos1 = [px0, py0, px1, py0 + pdy]
pos2 = [px0, py0 + pdy + pgap, px1, py1]
...
plot, xr1, ylog=ylog, /nodata, charsize=cs, font=fnt, position=pos1, $
	xstyle=1, xrange=xr2, ystyle=1, yrange=yr, $
	xtickinterval=100, $
	xtitle='MJD - ' + string(MJD0, format='(I0)'), $
	ytitle=strjoin(replicate(' ', 60)) + '0.3-2 keV Luminosity (10!U39!N erg s!U-1!N)'
...
plot, xr2, ylog=ylog, /nodata, charsize=cs, font=fnt, position=pos2, /noerase, $
